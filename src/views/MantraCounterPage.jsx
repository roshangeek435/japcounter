...
import { getMantrasByCategory } from "../lib/mantras";

export const ScriptFontClass = (text) => {
...
export default function MantraCounterPage({ mantra }) {
	...
	return (
		<>
			<section
				...
				data-testid={`mantra-hero-${mantra.id}`}
			>
				...
				<div className="text-center max-w-4xl mx-auto">
					...
					<div className="flex flex-wrap justify-center gap-2 mt-4">
						<Badge
							variant="outline"
							className="border-[#D4AF37] text-[#7B1C1C]"
						>
							Deity: {mantra.deity}
						</Badge>
						<Badge
			...
			<section
				className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid md:grid-cols-2 gap-6"
				data-testid={`mantra-info-${mantra.id}`}
			>
			...
			{related.length > 0 && (
				<section
					...
					data-testid={`related-mantras-${mantra.id}`}
				>
					...
					<div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
						{related.map((r) => (
							...
							>
								...
								<p className="text-xs uppercase tracking-widest text-[#D4AF37] mt-2 inline-flex items-center gap-1">
			...
			<section
				...
				data-testid={`mantra-faq-${mantra.id}`}
			>
				...
				<div className="space-y-4">
					{buildFAQ(mantra).map((f, i) => (
						<details
							key={i}
							className="bg-white border border-[#D4AF37]/30 rounded-2xl p-5 group"
			...
			<section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pb-14 text-center">
				<Button
					asChild
					variant="outline"
					className="rounded-full border-[#7B1C1C] text-[#7B1C1C]"
					data-testid="back-to-library-btn"
				>
					<Link href="/mantra-library">← Browse all mantras</Link>
				</Button>
			</section>
		</>
	);
