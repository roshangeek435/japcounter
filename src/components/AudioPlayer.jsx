...
const Equalizer = () => (
  <span className="inline-flex items-end gap-[2px] h-4 ml-2" aria-hidden="true">
    {[0,1,2].map(i => (
      <span key={i}
...
export const AudioPlayer = ({
  ...
  useEffect(() => { setTrackId(defaultTrack); }, [defaultTrack]);
  ...
  return (
    <div className="bg-white border border-[#D4AF37]/30 rounded-2xl p-5 shadow-sm" data-testid="audio-player">
      ...
      <Select value={trackId} onValueChange={onTrackChange}>
        <SelectTrigger className="border-[#D4AF37]/40 bg-[#FFF8EE]" data-testid="audio-track-select">
          <SelectValue />
        </SelectTrigger>
        <SelectContent className="bg-[#FFF8EE] border-[#D4AF37]/40">
          {TRACKS.map(t => (
            <SelectItem key={t.id} value={t.id} data-testid={`audio-track-${t.id}`}>{t.name}</SelectItem>
          ))}
        </SelectContent>
      </Select>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <Slider value={[Math.round(volume * 100)]} max={100} step={1} onValueChange={(v) => setVolume(v[0]/100)} className="flex-1" data-testid="volume-slider" />
      ...
      {error && (
        <p className="mt-2 text-xs text-amber-700 inline-flex items-center gap-1" data-testid="audio-error">
          <AlertCircle className="w-3 h-3" /> {error}
        </p>
      )}

      <div className="mt-4 flex items-center justify-between border-t border-[#D4AF37]/20 pt-3">
        <label className="text-sm text-[#1A1A1A]" htmlFor="click-sound">Tap click sound</label>
        <Switch id="click-sound" checked={clickSoundEnabled} onCheckedChange={setClickSoundEnabled} data-testid="click-sound-toggle" />
      </div>
    </div>
  );
};
